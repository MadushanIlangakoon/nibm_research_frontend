// client/src/components/TeacherEnrollmentRequests.js
import React, { useState, useEffect, useCallback } from 'react';
import axios from 'axios';
import { useAuth } from '../../context/AuthContext';
import { supabase } from '../../supabaseClient';

const TeacherEnrollmentRequests = ({onBack}) => {
    ...
    const fetchRequests = useCallback(async () => {
        if (user && user.id && teacherId) {
            try {
                const res = await axios.get(`${window.baseUrl}/api/enrollments/requests`, {
                    params: { teacher_id: teacherId },
                });
                console.log("Fetched enrollments:", res.data);
                let requestsData = res.data;
                console.log("Fetched enrollments:", requestsData);
                ...
                const mergedRequests = requestsData.map((req) => ({
                    ...req,
                    student: { name: studentMap[req.student_id] || 'Unknown Student' },
                }));
    ...
    // Function to fetch teacher courses and teacherId.
    ...
    const updateStatus = async (enrollment_id, status) => {
        try {
            await axios.post(`${window.baseUrl}/api/enrollments/update`, { enrollment_id, status });
            fetchRequests();
        } catch (error) {
    ...
    return (
        <div className="p-4 mt-8">
            ...
            <h3 className="text-2xl font-bold text-gray-800 mb-6 mt-6">Enrollment Requests</h3>
            ...
            <div className="mb-8">
                <h4 className="text-xl font-semibold text-gray-700 mb-4">Rejected Enrollment Requests</h4>
                {rejectedRequests.length === 0 ? (
                    <p className="text-gray-500">No rejected enrollment requests.</p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
