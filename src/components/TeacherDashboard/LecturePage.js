...
const LecturePage = () => {
    ...
    useEffect(() => {
        const fetchExpectedStudents = async () => {
            try {
                const response = await axios.get(`${window.baseUrl}/api/students`);
    ...
    const logAndSendStats = async (meetingEndTime) => {
        ...
        for (const [studentId, stats] of Object.entries(studentStatsRef.current)) {
            ...
            try {
                await axios.post(`${window.baseUrl}/api/lecture_statistics`, payload);
            } catch (err) {
        ...
        try {
            await axios.patch(`${window.baseUrl}/api/lectures/update_average_gaze`, {
                lecture_id: lecture.id,
    ...
    const handleEndMeeting = async () => {
        ...
        try {
            await axios.post(`${window.baseUrl}/api/lectures/end`, {
                lecture_id: lecture.id,
    ...
    useEffect(() => {
        async function fetchLecture() {
            try {
                const res = await axios.get(`${window.baseUrl}/api/lectures/${lectureId}`);
                setLecture(res.data);
            } catch (err) {
    ...
    useEffect(() => {
        ...
        socketRef.current = io.connect(`${window.baseUrl}`, {transports: ["websocket"]});
        ...
        navigator.mediaDevices
            ...
            .then((stream) => {
                ...
                socketRef.current.on("student-joined", async (studentDetails) => {
                    try {
                        await axios.post(`${window.baseUrl}/api/lectures/${lecture.id}/participants`, {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-gray-50">
                    ...
                    {role === "teacher" && showDrawer && (
                        <div
                            ...
                            <div className="p-4 h-full overflow-y-auto relative">
                                ...
                                {students.length > 0 && (
                                    <div className="mb-4">
                                        <h3 className="text-md mb-1">Active Viewing</h3>
            ...
            {/* Chat component below video call view, always visible */}
            <div className="w-11/12 p-4 bg-gray-50">
                <CollapsibleChat lecture_id={lecture.id} user={user} />
            </div>
